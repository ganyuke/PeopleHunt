...
class PaperSchedulerAdapter(private val plugin: JavaPlugin) : SchedulerPort {
    override fun everyTicks(interval: Long, task: () -> Unit): TaskHandle {
        val handle = plugin.server.scheduler.runTaskTimer(plugin, Runnable { task() }, 0L, interval)
        return object : TaskHandle { override fun cancel() = handle.cancel() }
    }
